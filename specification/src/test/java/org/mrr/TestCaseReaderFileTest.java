...
public class TestCaseReaderFileTest {
  ...
  public void whenReadingFromEmptyFile_shouldReturnEmptyList() {
      final List<String> testCases = new FileTestSpecificationStore(TestConstants.TEST_RESOURCE_FOLDER_LOCATION + "/EmptyTestCase.txt").testDescriptions();
    assertThat(testCases).isEmpty();
  }

  @Test
  public void whenReadingFromSampleFile_shouldReturnExpectedListElements() {
      final List<String> testCasesFromFile = new FileTestSpecificationStore(TestConstants.TEST_RESOURCE_FOLDER_LOCATION + "FirstTestCase.txt").testDescriptions();
